...
use std::fmt:: Write;
use crate::{jsonfixer_error::SyntaxError, jsonformatter::Formatter, JsonFixerConfig, JsonFixerError, JsonFormatter, JsonTokenizer, Token};
...
impl JsonValue {
    fn as_array(&self) -> Option<&Vec<JsonEntryValue>> {
        match self {
            Self::Array(arr) => Some(arr),
            _ => None,
        }
    }
    fn as_object(&self) -> Option<&Vec<JsonEntryValue>> {
        match self {
...
impl<'a> JsonParser<'a> {
    ...
    pub fn parse(&mut self) -> Result<String, JsonFixerError> {
        ...
        write!(output, "{}", formetter.format(&value, &config)?).map_err(|err| JsonFixerError::IO(err))?;
    ...
    fn parse_value(&mut self,) -> Result<JsonValue, JsonFixerError> {
        ...
        match &self.current_token {
            ...
            Some(Token::String(s, _)) => {
                ...
                Ok(JsonValue::String(s.replace('"', "\\\"")))
            }
            Some(Token::Number(n, pos)) => {
                let _result: f64 = n
                    .parse()
                    .map_err(|_| JsonFixerError::Syntax(SyntaxError::InvalidNumber(n.clone(), pos.clone())))?;
            ...
            None => {
                Err(JsonFixerError::Syntax(SyntaxError::UnexpectedEndOfInput(
    ...
    fn parse_object(&mut self,) -> Result<JsonValue, JsonFixerError> {
        ...
        while !self.current_token.is_none() {
            ...
            match &self.current_token {
                ...
                Some(Token::Comma(_)) => { // Empty entry
            ...
            // parse key
            match &self.current_token {
                Some(Token::RightBrace(_)) => { // Empty object with inside spaces eg. {   }
                    entry.value = None;
                    obj.push(entry);
                    break;
                },
    ...
    fn parse_array(&mut self,) -> Result<JsonValue, JsonFixerError> {
       let mut arr = Vec::new();
        ...
        while !self.current_token.is_none() {
            ...
            match &self.current_token {
                Some(Token::RightBracket(_)) => {// Empty array with spaces inside it
                    entry.value = None;
                    arr.push(entry);
                    break;
                }
                Some(Token::Comma(_)) => {// Empty array with spaces inside it and commas
                    // Consume consecutive commas (e.g., [,,])
                    entry.value = None;
                    arr.push(entry);
                    self.advance()?;
